...
- **DTE-022:** explicitly upload the hidden build artifact, prove it is a nonempty no-follow regular file with size and digest evidence, and create the image context, archive metadata, and evidence output only through confined no-link directory trees without replacing an existing evidence file.
...
4. Hidden OCI artifacts upload with a nonzero size and digest; EAI evidence and standard attestation identify the same subject. Image-context preparation, archive inspection, metadata, and evidence creation reject a linked or non-directory ancestor, linked or existing evidence files, and paths outside the application root, including an application-controlled `.eai-build` replacement after the build.
