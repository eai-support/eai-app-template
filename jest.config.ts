...
const config: Config = {
  ...
  // SECURITY: PR validation may contain a read-only, exact-commit producer
  // checkout under .ci. It is supply-chain input, never template test input.
  testPathIgnorePatterns: ['<rootDir>/.next/', '<rootDir>/.ci/'],
  watchPathIgnorePatterns: ['<rootDir>/.next/', '<rootDir>/.ci/'],
  moduleNameMapper: {
    '^@/(.*)$': '<rootDir>/src/$1',
    '^@enterpriseaigroup/platform-sdk$':
      '<rootDir>/packages/platform-sdk/src/index.ts',
    '^@enterpriseaigroup/platform-sdk/(.*)$':
      '<rootDir>/packages/platform-sdk/src/$1',
  ...
  transformIgnorePatterns: [
    ...
    'node_modules/(?!@azure/web-pubsub-client)',
  ],
};
